...
namespace TreeCounterAddin
{
    ...
    public class UiTextConverter : IValueConverter
    {
        public static readonly Dictionary<string, (string En, string Id)> Text = new()
        {
            ...
            ["Firms_Hint"] = (
                "Loads satellite-detected fire hotspots (NASA FIRMS) over the current map extent - cross-check against Land Clearing Detection results, since burning is a common land-clearing method. \"All Sources\" (default) queries 4 satellites (VIIRS x3 + MODIS) and merges results - one alone can miss a fire the others catch. Shown as a heat map (density blob, not individual dots) - re-symbolize the layer yourself (right-click > Symbology) for individual points or other styles. Also adds a dense field of small blue dashes (fixed screen size, stays small at any zoom, Windy.com-style) across the same extent showing current wind direction (smoke drift), from Open-Meteo - free, no key needed. Needs a free MAP_KEY on the Settings tab.",
                "Memuat titik panas dari satelit (NASA FIRMS) di sekitar extent map saat ini - cocok untuk cross-check hasil Land Clearing Detection, karena membakar adalah metode pembukaan lahan yang umum. \"All Sources\" (default) query 4 satelit sekaligus (VIIRS x3 + MODIS) lalu digabung - satu satelit saja bisa melewatkan titik yang tertangkap satelit lain. Ditampilkan sebagai heat map (area gradasi kepadatan, bukan titik satu-satu) - ganti sendiri simbologinya (klik kanan layer > Symbology) kalau mau tampilan titik individual atau gaya lain. Juga menambahkan medan padat garis-garis kecil biru (ukuran tetap di layar, tetap kecil di zoom berapa pun, gaya Windy.com) di area yang sama menunjukkan arah angin saat ini (arah sebaran asap), dari Open-Meteo - gratis, tanpa key. Butuh MAP_KEY gratis di tab Settings."),
            ...
            ["FirmsSettings_Header"] = ("NASA FIRMS MAP_KEY", "NASA FIRMS MAP_KEY"),
            ["FirmsSettings_Hint"] = (
