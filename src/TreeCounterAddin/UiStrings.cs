...
namespace TreeCounterAddin
{
    ...
    public class UiTextConverter : IValueConverter
    {
        public static readonly Dictionary<string, (string En, string Id)> Text = new()
        {
            ...
            ["Firms_Hint"] = (
                "Loads satellite-detected fire hotspots (NASA FIRMS) over the current map extent - cross-check against Land Clearing Detection results, since burning is a common land-clearing method. \"All Sources\" (default) queries 4 data sources spanning 5 satellites (VIIRS on SNPP/NOAA-20/NOAA-21 + MODIS from Terra+Aqua combined) and merges results - one alone can miss a fire the others catch. Shown as a heat map (density blob, not individual dots) - re-symbolize the layer yourself (right-click > Symbology) for individual points or other styles. Needs a free MAP_KEY on the Settings tab.",
                "Memuat titik panas dari satelit (NASA FIRMS) di sekitar extent map saat ini - cocok untuk cross-check hasil Land Clearing Detection, karena membakar adalah metode pembukaan lahan yang umum. \"All Sources\" (default) query 4 sumber data yang mencakup 5 satelit (VIIRS di SNPP/NOAA-20/NOAA-21 + MODIS gabungan Terra+Aqua) lalu digabung - satu source saja bisa melewatkan titik yang tertangkap source lain. Ditampilkan sebagai heat map (area gradasi kepadatan, bukan titik satu-satu) - ganti sendiri simbologinya (klik kanan layer > Symbology) kalau mau tampilan titik individual atau gaya lain. Butuh MAP_KEY gratis di tab Settings."),
